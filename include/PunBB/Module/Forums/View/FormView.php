...
namespace PunBB\Module\Forums\View;
...
/**
 * The forums page or the form editing a forum as the template shows it, built
 * in the order the page is read: the markup placed at each position, and the
 * numbers the forms give their field groups, items and fields.
 */
final class FormView {
	...
	public function place(ForumsRendering|ForumFormRendering $event): void {
		$this->positions[$event->position()] = new Html($event->markup());
		$this->count($event->groupCount(), $event->itemCount(), $event->fieldCount());
	}

	/** What the observers of $event added inside a row, counting on from where they left the form. */
	public function placeRow(ListedForumRendering|GroupPermissionRendering $event): Html {
		$this->count($event->groupCount(), $event->itemCount(), $event->fieldCount());
