...
/**
 * A position in the login form, which an observer may add markup at. The form
 ...
 * still change; the errors are there only when something stopped the login.
 */
final class LoginRendering implements EventInterface {
	use PartsByName;

	public const OUTPUT_START = 'output_start';

	public const PRE_LOGIN_ERRORS = 'pre_login_errors';

	public const PRE_LOGIN_GROUP = 'pre_login_group';

	public const PRE_USERNAME = 'pre_username';

	public const PRE_PASS = 'pre_pass';

	public const PRE_REMEMBER_ME_CHECKBOX = 'pre_remember_me_checkbox';
	...
	public const POSITIONS = array(
		self::OUTPUT_START, self::PRE_LOGIN_ERRORS, self::PRE_LOGIN_GROUP, self::PRE_USERNAME, self::PRE_PASS,
		self::PRE_REMEMBER_ME_CHECKBOX, self::PRE_GROUP_END, self::GROUP_END, self::END,
	);
	...
	public function __construct(
		...
		if (!in_array($position, self::POSITIONS, true))
			throw new InvalidArgumentException(sprintf('The login form has no position "%s"', $position));
