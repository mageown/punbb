...
namespace PunBB\Module\Profile\View;
...
/**
 * A page of a profile as its template shows it, built in the order the page is
 * read: the markup placed at each position, and the numbers the page gives its
...
final class FormView {
	...
	/** @var array<string, int> what the page numbers => its number */
	...
	/** Places what the observers of $event added, and counts on from where they left the page. */
	public function place(ProfileRendering $event): Html {
		$this->recount($event);

		return $this->positions[$event->position()] = new Html($event->markup());
	}

	/** Counts on from where the observers of a forum of the moderators' list left the form. */
	public function recount(ProfileRendering|ModeratorForumRendering $event): void {
	...
	/** A part of the page numbering its items from one again. */
